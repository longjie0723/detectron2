...
Registry for multi-mask heads, which predicts instance masks given
per-region features.
...
def mask_rcnn_loss(pred_mask_logits, pred_secondary_mask_logits, instances, vis_period=0):
    """
    ...
    for instances_per_image in instances:
        ...
        gt_secondary_masks_per_image = instances_per_image.gt_masks.crop_and_resize(
            instances_per_image.proposal_boxes.tensor, mask_side_len
        ).to(device=pred_mask_logits.device)

        # A tensor of shape (N, M, M), N=#instances in the image; M=mask_side_len
        gt_masks.append(gt_masks_per_image)
        gt_secondary_masks.append(gt_secondary_masks_per_image)
